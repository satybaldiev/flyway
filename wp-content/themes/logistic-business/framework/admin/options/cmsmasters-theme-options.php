...
if (CMSMASTERS_LEARNPRESS) {
	locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-lpr-course.php', true);
	
	locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-lpr-quiz.php', true);
}


if (!function_exists('logistic_business_get_custom_all_meta_fields')) {
function get_custom_all_meta_fields() {
	...
	if ( 
		...
		(isset($_GET['post_type']) && $_GET['post_type'] == 'lpr_course') || 
		(isset($_POST['post_type']) && $_POST['post_type'] == 'lpr_course') || 
		(isset($_GET['post']) && get_post_type($_GET['post']) == 'lpr_course') 
	) {
		...
		(isset($_GET['post_type']) && $_GET['post_type'] == 'lpr_quiz') || 
		(isset($_POST['post_type']) && $_POST['post_type'] == 'lpr_quiz') || 
		(isset($_GET['post']) && get_post_type($_GET['post']) == 'lpr_quiz') 
	) {
		$custom_lpr_quiz_meta_fields = get_custom_lpr_quiz_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_lpr_quiz_meta_fields, $custom_meta_fields);
	} elseif ( 
...
function logistic_business_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_localize_script('cmsmasters_theme_options_js', 'cmsmasters_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'logistic-business'), 
			'select_format' => 		esc_attr__('Please select the format.', 'logistic-business'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'logistic-business'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'logistic-business'), 
...
add_action('admin_enqueue_scripts', 'logistic_business_admin_enqueue_scripts');
...
function show_cmsmasters_meta_box() {
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'logistic-business') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'logistic-business') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'logistic-business') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'logistic-business') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'logistic-business') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'logistic-business') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'logistic-business') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'logistic-business') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'logistic-business') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'logistic-business') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'logistic-business') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'logistic-business') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'logistic-business') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'logistic-business') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'logistic-business') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'logistic-business') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'logistic-business') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'logistic-business') : esc_attr__('Create Gallery', 'logistic-business')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'logistic-business') . '" data-button="' . esc_attr__('Insert Gallery', 'logistic-business') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'logistic-business') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'logistic-business') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'logistic-business') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function add_custom_cmsmasters_meta_box() {
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'logistic-business'), 
