...
function logistic_business_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	if (
...
function logistic_business_header_top($cmsmasters_option) { 
	if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_top_line']) {
				...
				'<div class="header_top_inner">';
					...
					if (
						...
						if (
							...
							logistic_business_social_icons();
						} elseif (
...
function logistic_business_header_mid($cmsmasters_option) {
	echo '<div class="header_mid" data-height="' . esc_attr($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_mid_height']) . '">' . 
		'<div class="header_mid_outer">' . 
			'<div class="header_mid_inner">' . 
				'<div class="logo_wrap">';
					logistic_business_logo();
				echo '</div>';
			
			if (
				$cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_styles'] != 'c_nav' && 
				$cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_button_check'] && 
				$cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_button_link'] != '' && 
				$cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_button_text'] != ''
			) {
				echo '<div class="header_mid_but_wrap">' . 
					'<div class="header_mid_but_wrap_inner">' . 
						'<a class="header_mid_but" href="' . esc_url($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_button_link']) . '">' . esc_html($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_button_text']) . '</a>' . 
					'</div>' . 
				'</div>';
			}
			...
			if (
				$cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_mid_search'] && 
				$cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_styles'] != 'c_nav'
			) {
				...
				'<div class="mid_nav_wrap">' . 
					'<nav role="navigation">';
						...
						$nav_args = array( 
							...
							'link_before' => 		'<span class="nav_item_wrap">', 
							'link_after' => 		'</span>', 
							'fallback_cb' => 		'logistic_business_fallback_menu' 
						);
...
function logistic_business_header_bot($cmsmasters_option) {
	if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_styles'] != 'default') {
		echo '<div class="header_bot" data-height="' . esc_attr($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_bot_height']) . '">' . 
			'<div class="header_bot_outer">' . 
				'<div class="header_bot_inner">' . 
					
					'<!-- _________________________ Start Bottom Search _________________________ -->';
					if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_header_bot_search']) {
						echo '<div class="search_wrap">' . 
							'<div class="search_wrap_inner">' . 
								'<div class="search_wrap_in_inner">';
									get_search_form();
					...
					echo '<!-- _________________________ Finish Bottom Search _________________________ -->' . 
					
					'<!-- _________________________ Start Navigation _________________________ -->' . 
					'<div class="resp_bot_nav_wrap">' . 
						'<div class="resp_bot_nav_outer">' . 
							'<a class="responsive_nav resp_bot_nav cmsmasters_theme_icon_resp_nav" href="javascript:void(0);">' . 
								'<span class="resp_nav_button"></span>' . 
							'</a>' . 
						'</div>' . 
					'</div>' . 
...
/* Get Logo Function */
function logistic_business_logo() {
	...
	if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_logo_type'] == 'text') {
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_logo_title'] != '') {
			$blog_title = stripslashes($cmsmasters_option['logistic-business' . '_logo_title']);
		} else {
		...
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_logo_subtitle'] != '') {
			$blog_descr = stripslashes($cmsmasters_option['logistic-business' . '_logo_subtitle']);
		} else {
			$blog_descr = (get_bloginfo('description')) ? get_bloginfo('description') : esc_html__('Default Logo Subtitle', 'logistic-business');
		}
		...
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_logo_url'] == '') {
			echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr(get_bloginfo('name')) . '" class="logo">' . "\n\t" . 
				...
				'<img class="logo_retina" src="' . esc_url(get_template_directory_uri()) . '/img/logo_retina.png" alt="' . esc_attr(get_bloginfo('name')) . '" width="169" height="56" />' . "\r" . 
			'</a>' . "\n";
			
			
			$logo_def_style_width = (int) ($header_mid_height * (169 / 56));
...
";
			...
			$logo_img_width = ((is_numeric($logo_img[0])) ? (int) $logo_img_url[1] : '169');
			
			$logo_img_height = ((is_numeric($logo_img[0])) ? (int) $logo_img_url[2] : '56');
...
";
			...
			if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_logo_url_retina'] != '') {
				...
				$logo_img_retina_width = ((is_numeric($logo_img_retina[0])) ? ((int) $logo_img_retina_url[1] / 2) : '169');
				
				$logo_img_retina_height = ((is_numeric($logo_img_retina[0])) ? ((int) $logo_img_retina_url[2] / 2) : '56');
...
function logistic_business_footer_logo($cmsmasters_option) {
	if (
		...
		if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_footer_logo_url'] == '') {
			echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr(get_bloginfo('name')) . '" class="footer_logo">' . "\n\t" . 
				...
				'<img class="footer_logo_retina" src="' . esc_url(get_template_directory_uri()) . '/img/logo_footer_retina.png" alt="' . esc_attr(get_bloginfo('name')) . '" width="169" height="56" />' . "\r" . 
			'</a>' . "\n";
			...
			if ($cmsmasters_option[CMSMASTERS_SHORTNAME . '_footer_logo_url_retina'] != '') {
				...
				$footer_logo_img_retina_width = ((is_numeric($footer_logo_img_retina[0])) ? ((int) $footer_logo_img_retina_url[1] / 2) : '169');
				
				$footer_logo_img_retina_height = ((is_numeric($footer_logo_img_retina[0])) ? ((int) $footer_logo_img_retina_url[2] / 2) : '56');
...
function logistic_business_get_footer_custom_html($cmsmasters_option) {
	if (
		(
			$cmsmasters_option[CMSMASTERS_SHORTNAME . '_footer_type'] == 'default' && 
...
function logistic_business_get_footer_nav($cmsmasters_option) {
	if (
		has_nav_menu('footer') && 
		(
			(
...
function logistic_business_get_footer_social_icons($cmsmasters_option) {
	if (
		isset($cmsmasters_option[CMSMASTERS_SHORTNAME . '_social_icons']) && 
		(
			(
...
function logistic_business_page_heading() {
	$cmsmasters_option = logistic_business_get_global_options();
			...
			if ($cmsmasters_heading == 'custom') {
				...
				if (is_search()) {
					...
					if (!empty($wp_query->found_posts)) {
						echo sprintf(esc_html(_n('1 search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'logistic-business')), $wp_query->found_posts, get_search_query());
					} else {
						echo sprintf(esc_html__('0 search results for: %s', 'logistic-business'), get_search_query());
					}
				} elseif (is_archive()) {
					if (is_author()) {
						if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
							echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'logistic-business'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
						} else {
							echo sprintf(esc_html__('Author: %s', 'logistic-business'), '<span class="vcard">' . get_the_author() . '</span>');
						}
					} else {
...
function logistic_business_social_icons() {
	$cmsmasters_option = logistic_business_get_global_options();
...
function logistic_business_theme_social_icons_styles() {
	$cmsmasters_option = logistic_business_get_global_options();
...
function logistic_business_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function logistic_business_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
function logistic_business_prev_next_posts() {
...
function logistic_business_sharing_box($title_box = false, $tag = 'h3') {
	...
	echo "<aside class=\"share_posts\">
		...
		<div class=\"share_posts_inner\">
			<a href=\"https://www.facebook.com/sharer/sharer.php?display=popup&u={$page_link}\" title=\"" . esc_html__('Facebook', 'logistic-business') . "\" class=\"cmsmasters-icon-facebook-circled\"></a>
			<a href=\"https://twitter.com/intent/tweet?text=" . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'logistic-business'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . "&url={$page_link}\" title=\"" . esc_html__('Twitter', 'logistic-business') . "\" class=\"cmsmasters-icon-twitter-circled\"></a>
			<a href=\"https://pinterest.com/pin/create/button/?url={$page_link}&media={$pinterest_img}&description={$social_title}\" title=\"" . esc_html__('Pinterest', 'logistic-business') . "\" class=\"cmsmasters-icon-pinterest-circled-1\"></a>
		</div>
	</aside>
";
}
...
function logistic_business_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_url) {
		echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'logistic-business') . '" target="_blank">' . esc_html($user_url) . '</a>';
	}
...
function logistic_business_related($tag = 'h3', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		echo "<aside class=\"cmsmasters_single_slider\">
			...
			if ($type == 'post') {
				esc_html_e('More posts', 'logistic-business');
			} else {
				esc_html_e('More projects', 'logistic-business');
			}
			...
			if ($r_query->have_posts()) {
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									logistic_business_get_post_date('page', 'related');
									
									logistic_business_thumb(get_the_ID(), 'cmsmasters-blog-masonry-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
									...
									echo "<div class=\"cmsmasters_single_slider_item_inner\">
										...
										if ($type == 'post') {
											logistic_business_get_post_category(get_the_ID(), 'category', 'related');
										} else {
											logistic_business_get_project_category(get_the_ID(), 'pj-categs', 'related');
										}
										
										echo "<div class=\"cmsmasters_single_slider_item_meta\">";
										
										if ($type == 'post') {
											logistic_business_get_post_likes('page');
											
											logistic_business_get_post_comments('page');
										} else {
											logistic_business_get_project_likes('page');
											
											logistic_business_get_project_comments('page');
										}
				...
				if ($type == 'post') {
					esc_html_e('No posts found', 'logistic-business');
				} else {
					esc_html_e('No projects found', 'logistic-business');
				}
...
function logistic_business_author_avatar($template_type = 'page') {
...
/* Get Category List */
function logistic_business_get_the_category_list($cmsmasters_id, $taxonomy, $sep = '', $before = '', $after = '') {
