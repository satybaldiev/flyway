...
<article id="post-<?php the_ID(); ?>" <?php post_class($cmsmasters_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		logistic_business_thumb_rollover(get_the_ID(), $project_img_size, $rollover, false, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url);
		...
		if ($rollover) {
			if ($title || $categories || $likes || $comments) {
				echo '<div class="project_inner">';
					...
					($title) ? logistic_business_project_heading(get_the_ID(), 'h6', true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url) : '';
					
					
					if ($categories) {
						echo '<div class="cmsmasters_project_cont_info entry-meta">';
							
							logistic_business_get_project_category(get_the_ID(), 'pj-categs', 'page');
