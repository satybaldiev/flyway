	...
	<?php 
		if (!post_password_required()) {
			if (sizeof($cmsmasters_post_images) >= 1 && $cmsmasters_post_images[0] != '') {
				...
				logistic_business_thumb(get_the_ID(), 'cmsmasters-project-thumb', false, 'img_' . get_the_ID(), true, false, true, true, $first_post_image, $unique_id);
				...
				if (sizeof($cmsmasters_post_images) > 0) {
					...
					foreach ($cmsmasters_post_images as $cmsmasters_post_image) {
						$image_src = wp_get_attachment_image_src(strstr($cmsmasters_post_image, '|', true), 'full');
				...
				logistic_business_thumb(get_the_ID(), 'cmsmasters-project-thumb', false, 'img_' . get_the_ID(), true, false, true, true, false);
			} else {
				logistic_business_post_format_icon_placeholder(get_the_ID(), 'gallery');
			}
		} else {
			logistic_business_post_format_icon_placeholder(get_the_ID(), 'gallery');
		}
