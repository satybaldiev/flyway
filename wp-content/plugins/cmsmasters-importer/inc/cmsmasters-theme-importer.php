...
if (!class_exists('Cmsmasters_Theme_Importer')) {
	class Cmsmasters_Theme_Importer {
		...
		public function demo_installer() {
			...
			echo '<div class="wrap">';
				...
				echo "<div class=\"cmsmasters-importer-notice cmsmasters-importer-notice-info\">
					...
					<ul>
						<li>" . __("No existing posts, pages, categories, images, custom post types or any other data excerpt theme settings will be deleted or modified.", 'cmsmasters-framework') . "</li>
						<li>" . __("Posts, pages, some images, theme settings, widgets, menus, sliders and and some other data will get imported.", 'cmsmasters-framework') . "</li>
		...
		public function set_demo_pages() {
			...
			if ($this->pages['show_on_front'] == 'page') {
				if ($this->pages['page_on_front'] != '') {
					$home_page = get_page_by_title($this->pages['page_on_front']);
				...
				if ($this->pages['page_for_posts'] != '') {
					$posts_page = get_page_by_title($this->pages['page_for_posts']);
		...
		function set_demo_woo_pages() {
			if (is_array($this->woo_pages)) {
				foreach ($this->woo_pages as $name => $title) {
					$woo_page = get_page_by_title($title);
					
					
					if ($woo_page && $woo_page->ID) {
						update_option($name, $woo_page->ID);
					}
				}
		...
		public function set_demo_sliders() {
			...
			if (is_dir($revslider) && class_exists('RevSlider')) {
				...
				if ($revslider_dir) {
					...
					while (($revslider_file = readdir($revslider_dir)) !== false){
						...
						$rev_slider->importSliderFromPost(true, true, $revslider . $revslider_file);
					}
